...
(function() {
    ...
    class Sequenced extends BMM.PT.Generic {
        ...
        executeActions() {
            ...
            for (var action of this.nextActions) {
                ...
                } else if (action[0] == "set_sprite") {
                    this.event.setImage(action[1], action[2]);
                } else if (action[0] == "nudge") {
                    BMM.TIME.pushNudge(action[1], action[2], action[3], action[4], action[5], action[6]);
    ...
    class Enemy extends Sequenced {
        ...
        damage(amount) {
            this.hp -= amount;
            var path = "bar_"+this.baseHP+"seg_"+this.hp;
            if (this.hp > 0) BMM.TIME.pushMapImage(this.event, path, 25, 25, 255, 1000);
    ...
    BMM.PT.spectreBlurStrengthMax = 0.075;
    BMM.PT.spectreBlurStrength = null;
    ...
    BMM.PT.BOSS_Spectre = class extends Boss {
        constructor(event) {
            ...
            this.attackDir = 0;
            this.flying = true;
            BMM.GLOBAL.interpreter.pluginCommand("createFilter", ["specblur", "zoomblur", 21, this.id]);
            BMM.GLOBAL.interpreter.pluginCommand("setFilter", ["specblur", "2", "-18", "0", BMM.PT.spectreBlurStrengthMax.toString()]);
            BMM.PT.spectreBlurStrength = new BMM.TIME.easeOut(BMM.PT.spectreBlurStrengthMax, 0, 2, 2000, true);
        ...
        reappear() {
            ...
            BMM.PT.spectreBlurStrength = new BMM.TIME.easeOut(BMM.PT.spectreBlurStrengthMax, 0, 2, 3000, true);
        }
        damage(amount) {
            ...
            if (this.hp <= 0) {
                ...
                if (this.stage > 2) {
                    ...
                    if (this.stage == 1) {
                        ...
                        $gameMessage.newPage();
                        $gameMessage.add("RISE \\C[10]MINIONS\\C AND DEFEND THE SEAL!");
                        ...
                        $gameMessage.newPage();
                        $gameMessage.add("\\C[30]Shroud of night\\C, protect the \\C[14]seal\\C!");
                    }
                    this.ghostlyDash();
                }
            } else {
    ...
    BMM.PT.radiantDash = null;
    BMM.PT.radiantDashDir = 0;
    BMM.PT.radiantTriedPunching = false;
    ...
    BMM.PT.BOSS_Radiant = class extends Boss {
        ...
        getNextActions() {
            ...
            if (this.isLunge || (this.stage >= 2 && this.lungeCooldown <= 0 && !this.isBomb && !this.isAttack)) {
                this.lungeSequence();
            } else if (this.isBomb || (this.stage >= 1 && this.bombCooldown <= 0 && !this.isLunge && !this.isAttack)) {
                this.getNextMove();
                this.bombSequence();
            } else if (this.isAttack || (this.stage >= 0 && this.distance[0] + this.distance[1] <= 1 && !this.isLunge && !this.isBomb)) {
                this.standardAttack();
            } else {
                this.getNextMove();
            }
        ...
        radiantBomb() {
            ...
            for (var loc of this.bossBombLocations) {
                if (this.x == loc[0] && this.y == loc[1]) continue; // Don't spawn on top of Radiant, leads to bug with hitting
            ...
            if (this.lungeState == 0) {
                ...
                if (this.bossDashDisplayLoc != null) {
                    ...
                    if (this.dashDirection == "up") {
                        tileInterpreter.pluginCommand('DISPLAYOVERLAYTILE', ['2386', '91', this.bossDashDisplayLoc[0], this.bossDashDisplayLoc[1]]);
                    } else if (this.dashDirection == "right") {
                        tileInterpreter.pluginCommand('DISPLAYOVERLAYTILE', ['2386', '89', this.bossDashDisplayLoc[0], this.bossDashDisplayLoc[1]]);
                    } else if (this.dashDirection == "down") {
                        tileInterpreter.pluginCommand('DISPLAYOVERLAYTILE', ['2386', '92', this.bossDashDisplayLoc[0], this.bossDashDisplayLoc[1]]);
                    } else {//direction is left
                        tileInterpreter.pluginCommand('DISPLAYOVERLAYTILE', ['2386', '90', this.bossDashDisplayLoc[0], this.bossDashDisplayLoc[1]]);
                    }
                } else {
        ...
        bossDashAndDamagePlayer() {
            ...
            if (this.dashDirection == "up" || this.dashDirection == "down") {
                BMM.PT.radiantDashDir = 2;
                BMM.PT.radiantDash = new BMM.TIME.easeOut(this.y, this.bossDashDisplayLoc[1], 5, 200, true);
            } else {
                BMM.PT.radiantDashDir = 1;
                BMM.PT.radiantDash = new BMM.TIME.easeOut(this.x, this.bossDashDisplayLoc[0], 5, 200, true);
            }
            //this.event.setPosition(this.bossDashDisplayLoc[0], this.bossDashDisplayLoc[1]);
            this.x = this.bossDashDisplayLoc[0];
            this.y = this.bossDashDisplayLoc[1];
            //console.log(">> " + this.bossDashDisplayLoc[0] + ", " + this.bossDashDisplayLoc[1]);
        }
        ...
        standardAttack() {
            if (this.attackState == 0) {
                ...
                this.nextActions.push(["nudge", this.event, this.attackDir, 0.4, 250, 4, true]);
                this.isAttack = false;
                this.attackState = 0;
            }
        }
            ...
            if (this.hp <= 0) {
                ...
                if (this.stage > 2) {
                    ...
                    if (this.stage == 1) {
                        ...
                        $gameMessage.setBackground(0);
                        $gameMessage.setPositionType(2);
                        $gameMessage.add("The \\C[14]seal\\C must remain intact!");
                    } else if (this.stage == 2) {
                        AudioManager.playSe({name: 'BossPhase', pan: 0, pitch: 100, volume: 45});
                        $gameMessage.setFaceImage("Evil", 4);
                        $gameMessage.setBackground(0);
        ...
        death() {
            ...
            BMM.PT.radiantDashDir = 0;
            BMM.PT.radiantDash = null;
            BMM.GLOBAL.interpreter.pluginCommand("eraseFilter", ["radrays"]);
            BMM.HYB.resetOverlays();
            for (var e of BMM.TRAN.level.events) {
                if (e.type == "projectile") e.destroy();
            }
            BMM.TRAN.level.scanEvents();
            $gameSelfSwitches.setValue([$gameMap._mapId, this.id, "A"], true);
    ...
    BMM.PT.BOSS_Deceiver = class extends Boss {
        ...
        getNextActions() {
            ...
            if ((this.isBurrow || this.burrowCooldown <= 0) && !this.isBomb && !this.isLunge && !this.isAttack){
                this.burrowSequence();
            } else if (this.isBomb || (this.stage >= 1 && this.bombCooldown <= 0 && !this.isBurrow && !this.isLunge && !this.isAttack)) {
                this.bombSequence();
                this.getNextMove();
            } else if (this.isAttack || (this.stage >= 0 && this.distance[0] + this.distance[1] <= 1 && !this.isBurrow && !this.isLunge && !this.isBomb)) {
                this.standardAttack();
        ...
        sequenceOfLaserAttack() {
            ...
            if(this.laserAttackStep == 0){//wait one turn
                ...
                // }
                /*if( this.laserSoundEffectPlaying == false )//Johnas's code for the laser se.
                {
                ...
                if( this.laserDetoSoundEffectPlaying == false )
                {
                    ...
                    AudioManager.playSe({name: 'DeceiverLaserDetonateLong', pan: 0, pitch: 100, volume: 50});
        ...
        warnLaserAttack(){
            ...
            for(var loc of this.warnLaserLocations){
                if(testDisplayTileValidity(loc[0], loc[1])){//only if you can walk here would you draw a tile overlay here
                    tileInterpreter.pluginCommand('DISPLAYOVERLAYTILE', ['2386', '3', loc[0], loc[1]]);
                }
            }
        }
        calcLaserPositions(){
            ...
            if(this.laserFiringStep == 0){//for calculatiung pfWarn of original warning laser positionns
                pfWarn = [this.x + 1, this.y - 5];
                //this.laserFiringStep++;
            } else if(this.laserFiringStep == 1){
                pfWarn = [this.x + 2, this.y - 5];
                //this.laserFiringStep++;
            }else if(this.laserFiringStep == 2){
                pfWarn = [this.x + 3, this.y - 5];
                //this.laserFiringStep++;
            } else if(this.laserFiringStep == 3){
                pfWarn = [this.x + 4, this.y - 5];
                //this.laserFiringStep++;
            }else if(this.laserFiringStep == 4) {
                pfWarn = [this.x + 5, this.y - 5];
                //this.laserFiringStep++;
            }else if(this.laserFiringStep == 5){
                pfWarn = [this.x + 5, this.y - 4];
                //this.laserFiringStep++;
            }else if(this.laserFiringStep == 6){
                pfWarn = [this.x + 5, this.y - 3];
                //this.laserFiringStep++;
            }else if(this.laserFiringStep == 7){
                pfWarn = [this.x + 5, this.y - 2];
                //this.laserFiringStep++;
            }else if(this.laserFiringStep == 8){//it's 8
                pfWarn = [this.x + 5, this.y - 1];
                //this.laserFiringStep++;
            }else {
                pfWarn = [this.x + 5, this.y];
                //this.laserFiringStep = 0;
            }

            if(this.laserFiringStep == 0){//for calculating pfDiagonalWarn starting from a diagonal position
                pfDiagonalWarn = [this.x + 5, this.y - 4];
                //this.laserFiringStep++;
            } else if(this.laserFiringStep == 1){
                pfDiagonalWarn = [this.x + 5, this.y - 3];
                //this.laserFiringStep++;
            }else if(this.laserFiringStep == 2){
                pfDiagonalWarn = [this.x + 5, this.y - 2];
                //this.laserFiringStep++;
            } else if(this.laserFiringStep == 3){
                pfDiagonalWarn = [this.x + 5, this.y - 1];
                //this.laserFiringStep++;
            }else if(this.laserFiringStep == 4) {
                pfDiagonalWarn = [this.x + 5, this.y];
                //this.laserFiringStep++;
            }else if(this.laserFiringStep == 5){
                pfDiagonalWarn = [this.x + 5, this.y + 1];
                //this.laserFiringStep++;
                ...
                pfDiagonalWarn = [this.x + 5, this.y + 5];
                //this.laserFiringStep = 0;
            }

            if(this.laserFiringStep == 0){//for calculating pfDiagonal starting from a diagonal position
            ...
            //laser warning next turn
            var xDiffWarn = pfWarn[0] - pi[0];
            var yDiffWarn = pfWarn[1] - pi[1];
            var originalXWarn = -1;
            var originalYWarn = -1;
            var tempXWarn = -1;
            var tempYWarn = -1;

            //diagonal laser warning next turn 
            var xDiffDiagonalWarn = pfDiagonalWarn[0] - pi[0];
            var yDiffDiagonalWarn = pfDiagonalWarn[1] - pi[1];
            var originalXDiagonalWarn = -1;
            var originalYDiagonalWarn = -1;
            var tempXDiagonalWarn = -1;
            var tempYDiagonalWarn = -1;
            ...
            for (var t = 0.2; t <= this.laserMaxLength; t+=0.2){
                //////////////////////////////////////////////////////////Warning laser display spawn locations
                //var tFuture = t + 0.2;
                if(this.x == pfWarn[0]){//edge case where formula does not work
                    originalXWarn = this.x;
                    originalYWarn = this.y - edgeCaseCounter - 1;//-1 to account for +1 above
                }else{//normal case
                    originalXWarn = Math.round(pi[0] + t*xDiffWarn);
                    originalYWarn = Math.round(pi[1] + t*yDiffWarn);
                }
                this.futureLaserAttackLocations.push([originalXWarn, originalYWarn]);//up position

                var firstTempXWarn = (originalYWarn - this.y) + this.x;
                var firstTempYWarn = -(originalXWarn - this.x) + this.y;
                this.futureLaserAttackLocations.push([firstTempXWarn, firstTempYWarn]);//counter-clock 90

                tempXWarn = (firstTempYWarn - this.y) + this.x;
                tempYWarn = -(firstTempXWarn - this.x) + this.y;
                this.futureLaserAttackLocations.push([tempXWarn, tempYWarn]);//180

                tempXWarn = -(originalYWarn - this.y) + this.x;
                tempYWarn = (originalXWarn - this.x) + this.y;
                this.futureLaserAttackLocations.push([tempXWarn, tempYWarn]);//clock 90
                ///////////////////////////////////////////////////////////////////////////////////////////////

                ///////////////////////////////////////////////////////////////////Diagonal laser spawn locations
                originalXDiagonalWarn = Math.round(pi[0] + t*xDiffDiagonalWarn);
                originalYDiagonalWarn = Math.round(pi[1] + t*yDiffDiagonalWarn);

                this.futureLaserAttackLocations.push([originalXDiagonalWarn, originalYDiagonalWarn]);

                var firstTempXDiagonalWarn = (originalYDiagonalWarn - this.y) + this.x;
                var firstTempYDiagonalWarn = -(originalXDiagonalWarn - this.x) + this.y;
                this.futureLaserAttackLocations.push([firstTempXDiagonalWarn, firstTempYDiagonalWarn]);//counter-clock 90

                tempXDiagonalWarn = (firstTempYDiagonalWarn - this.y) + this.x;
                tempYDiagonalWarn = -(firstTempXDiagonalWarn - this.x) + this.y;
                this.futureLaserAttackLocations.push([tempXDiagonalWarn, tempYDiagonalWarn]);//180

                tempXDiagonalWarn = -(originalYDiagonalWarn - this.y) + this.x;
                tempYDiagonalWarn = (originalXDiagonalWarn - this.x) + this.y;
                this.futureLaserAttackLocations.push([tempXDiagonalWarn, tempYDiagonalWarn]);//clock 90
                ///////////////////////////////////////////////////////////////////////////////////////////////   
        ...
        showLaserAttackDisplays() {
            // && !this.allLaserAttackDisplayLocations.includes([loc[0], loc[1]])
            var tileInterpreter = new Game_Interpreter();
            for(var loc of this.futureLaserAttackLocations){//future laser attacks, not the first warning of them
                if(testDisplayTileValidity(loc[0], loc[1])) {//only if you can walk here would you draw a tile overlay here
                    tileInterpreter.pluginCommand('DISPLAYOVERLAYTILE', ['2386', '3', loc[0], loc[1]]);
                }
            }
        ...
        shockwave() {
            BMM.GLOBAL.interpreter.pluginCommand("createFilter", ["deshock", "shockwave", 31, this.id]);
            BMM.GLOBAL.interpreter.pluginCommand("setFilter", ["deshock", this.x, this.y, 260, 50, 50, 0.75]);
            BMM.GLOBAL.interpreter.pluginCommand("setFilterSpeed", ["deshock", 0.025]);
        }
        findEmergeLocation() {
            var playerX = $gamePlayer.x;
            var playerY = $gamePlayer.y;
        ...
        warnSpinAttack() {
            ...
            this.spinAttackDisplayLocations = []; // 8 per row, starting at 19
            this.spinAttackDisplayLocations.push([this.x - 1, this.y, "27"]);       // left
            this.spinAttackDisplayLocations.push([this.x - 1, this.y - 1, "20"]);   // up left
            this.spinAttackDisplayLocations.push([this.x, this.y - 1, "19"]);       // up
            this.spinAttackDisplayLocations.push([this.x + 1, this.y - 1, "44"]);   // up right
            this.spinAttackDisplayLocations.push([this.x + 1, this.y, "43"]);       // right
            this.spinAttackDisplayLocations.push([this.x + 1, this.y + 1, "36"]);   // down right
            this.spinAttackDisplayLocations.push([this.x, this.y + 1, "35"]);       // down
            this.spinAttackDisplayLocations.push([this.x - 1, this.y + 1, "28"]);   // down left

            var tileInterpreter = new Game_Interpreter();
            for(var loc of this.spinAttackDisplayLocations){
                if(testDisplayTileValidity(loc[0], loc[1])){//only if you can walk here would you draw a tile overlay here
                    tileInterpreter.pluginCommand( 'DISPLAYOVERLAYTILE', [ '2386', '11', loc[0], loc[1] ]);
                }
            }
            
        }
        doSpinAttack() {
            AudioManager.playSe({name: 'DeceiverSpin', pan: 0, pitch: 100, volume: 90});
            var playerX = $gamePlayer.x;
            var playerY = $gamePlayer.y;
            var tileInterpreter = new Game_Interpreter();
            for(var loc of this.spinAttackDisplayLocations){
                if(testDisplayTileValidity(loc[0], loc[1])){//only if you can walk here would you draw a tile overlay here
                    tileInterpreter.pluginCommand( 'DISPLAYOVERLAYTILE', [ '2386', loc[2], loc[0], loc[1] ]);
                    if (playerX == loc[0] && playerY == loc[1]){
                        BMM.HYB.playerDamage(1);
                    }
                }
        ...
        warnTentacleAttack() {
            ...
            for(var loc of this.tentacleAttackDisplayLocations){
                if(testDisplayTileValidity(loc[0], loc[1])){//only if you can walk here would you draw a tile overlay here
                    tileInterpreter.pluginCommand( 'DISPLAYOVERLAYTILE', [ '2386', '5', loc[0], loc[1] ]);
                }  
            }
        }
        doTentacleAttack() {
           ...
           var tileInterpreter = new Game_Interpreter();
            for(var loc of this.tentacleAttackDisplayLocations){
                if(testDisplayTileValidity(loc[0], loc[1])){//only if you can walk here would you draw a tile overlay here
                    tileInterpreter.pluginCommand( 'DISPLAYOVERLAYTILE', [ '2386', '7', loc[0], loc[1] ]);
                    if (playerX == loc[0] && playerY == loc[1]){
                        BMM.HYB.playerDamage(1);
                    }
                }
        ...
        standardAttack() {
            ...
            if (this.attackState == 0) {
                ...
                var pos = dirToMove(this.attackDir);
                this.nextActions.push(["play_se", "DeceiverMelee", 0, 100, 100]);
                this.nextActions.push(["hit", this.x + pos[0], this.y + pos[1], this.atk, ["enemy"]]);
                this.nextActions.push(["nudge", this.event, this.attackDir, 0.4, 250, 4, true]);
                this.isAttack = false;
                this.attackState = 0;
            ...
            if (this.hp <= 0) {
                ...
                if (this.stage > 3) {
                    ...
                    if (this.stage == 1) {
                        ...
                        $gameMessage.setBackground(0);
                        $gameMessage.setPositionType(2);
                        $gameMessage.add("Time is getting \\C[14]faster\\C, I'm almost free.\nJust give me the \\C[14]seal\\C!");
                    } else if (this.stage == 2) {
                        AudioManager.playSe({name: 'BossPhase', pan: 0, pitch: 100, volume: 45});
                        $gameMessage.setFaceImage("Evil", 2);
                        $gameMessage.setBackground(0);
                        ...
                        $gameMessage.newPage();
                        $gameMessage.add("You \\C[10]ANIMAL\\C!")
                        $gameMessage.newPage();
                        ...
                        $gameMessage.newPage();
                        $gameMessage.add("You'll be trapped here \\C[30]forever\\C.");
                        $gameMessage.newPage();
        ...
        death() {
            ...
            $gameMap.changeTileset(8);
            $gameSelfSwitches.setValue([$gameMap._mapId, this.id, "B"], true);
            AudioManager.playSe({name: 'DeceiverKillDeath', pan: 0, pitch: 100, volume: 55});
            AudioManager.stopBgm();
            $gameScreen.erasePicture( 20 );
            $gameScreen.erasePicture( 21 );
            $gameScreen.erasePicture( 22 );
    ...
    BMM.PT.PROJ_Bomb = class extends Projectile { // TODO
        ...
        changeBombState(){
            ...
            } else if (this.fuse == 2) {//Ready to explode
    ...
    BMM.PT.ENEMY_Melee = class extends Enemy {
        constructor(event, baseHP, attack, mods) {
            ...
            this.state = 0; // 0 = moving, 1 = attacking
            this.distance = 999;
            this.originalSprite = this.event._characterName; 
            this.originalIndex = this.event._characterIndex;
        }
        getNextActions() {
            this.getState();
            if (this.aggro == false) {
        ...
        getState() {
            ...
            if (this.distance[0] + this.distance[1] <= 5) {
                if (this.aggro == false) BMM.TIME.pushMapImage(this.event, "noticed_small", 20, 20, 255, 500);
                this.aggro = true;
            }
        ...
        attack() {
            ...
            if (this.attackInd == 0) {
                ...
                this.nextActions.push(["face", this.attackDir]);
                this.nextActions.push(["warn", this.x + pos[0], this.y + pos[1], "attack"]);
                this.nextActions.push(["set_sprite", this.originalSprite, this.originalIndex + 4]);
            } else if (this.attackInd == 1) {
                this.attackInd += 1;
                var pos = dirToMove(this.attackDir);
                this.nextActions.push(["hit", this.x + pos[0], this.y + pos[1], this.atk, ["boss", "enemy"]]);
                this.nextActions.push(["nudge", this.event, this.attackDir, 0.4, 200, 2, true]);
                this.nextActions.push(["set_sprite", this.originalSprite, this.originalIndex]);
                this.attackInd = 0;
                this.state = 0;
            }
        }
