...
(function() {
    ...
    Filter_Controller.prototype.getCharLoc = function() {
        ...
        if(!!this._charMapId) { // search event screen Loc
            if(this._character < 0) return [$gamePlayer.screenX() * $gameMap.zoom.x, $gamePlayer.screenY() * $gameMap.zoom.y];
            else {
                char = $gameMap.event(this._character);
                if(char) return [char.screenX() * $gameMap.zoom.x, char.screenY() * $gameMap.zoom.y];
            }
        }
