...
(function() {
        ...
        spawnMinions() {
            ...
            for (var pos of this.spawnLocations) {
                if (this.stage == 1) {
                    ...
                    this.nextActions.push(["spawn_event", pos[0], pos[1], "melee_elite", '{"type":"enemy", "class":"melee", "basehp":2, "attack":1, "mod":["aggro", "spawned"]}']);
                }
            }
            this.spawnLocations = [];
        }
            ...
            if (this.hp <= 0) {
                ...
                if (this.stage > 2) {
                    ...
                    if (this.stage == 1) {
                        ...
                        $gameMessage.setFaceImage("Monster", 6);
    ...
    BMM.PT.BOSS_Radiant = class extends Boss { // TODO
        ...
        bombSequence() {
            if (this.bombState == 0) {
                //working code for bombs. Just commented out so i can test other abilities
    ...
    BMM.PT.BOSS_Deceiver = class extends Boss {
        ...
        getNextActions() {
            if ($gameSelfSwitches.value([$gameMap._mapId, this.id, "C"]) == true) this.destroy();
            this.x = this.event._x;
            this.y = this.event._y;
            if (this.stage >= 3) {
        ...
        sequenceOfLaserAttack() {
            ...
            if(this.laserAttackStep == 0){//wait one turn
                ...
                this.laserAttackStep++;
                tileInterpreter.pluginCommand('DISPLAYOVERLAYTILE', ['2386', '4', this.bossEmergeLoc[0], this.bossEmergeLoc[1]]);
        ...
        damage(amount) {
            ...
            if (this.hp <= 0) {
                ...
                if (this.stage > 3) {
                    ...
                    if (this.stage == 1) {
                        AudioManager.playSe({name: 'BossPhase', pan: 0, pitch: 100, volume: 45});
                        $gameMessage.setFaceImage("Evil", 5);
                        $gameMessage.setBackground(0);
                        $gameMessage.setPositionType(2);
