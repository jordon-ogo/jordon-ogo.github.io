...
(function() {
    ...
    BMM.TIME.dashInterp = null;
    BMM.TIME.dashDir = 0;
    BMM.TIME.playerEffectivePosition = [null, null];
    ...
    BMM.TIME.activateDash = function(direction) {
        if (potentialDashMove[direction][0] != -1){//if it equals -1, cannot dash in direction
            ...
            if (direction == "up" || direction == "down") {
                BMM.TIME.dashDir = 2;
                BMM.TIME.playerEffectivePosition = [$gamePlayer.x, potentialDashMove[direction][1]];
                BMM.TIME.dashInterp = new BMM.TIME.easeOut($gamePlayer.y, potentialDashMove[direction][1], 5, 250, true);
            } else if (direction == "left" || direction == "right") {
                BMM.TIME.dashDir = 1;
                BMM.TIME.playerEffectivePosition = [potentialDashMove[direction][0], $gamePlayer.y];
                BMM.TIME.dashInterp = new BMM.TIME.easeOut($gamePlayer.x, potentialDashMove[direction][0], 5, 250, true);
            }
    ...
    class Time {
        ...
        get() {
            if (BMM.GLOBAL.pauseTime && !this.ignorepause) {
                return this.paused();
            } else {
    ...
    BMM.TIME.easeOut = class extends Time {
        constructor(start, end, exp, time, ignore = false) {
            super(ignore);
            this.start = start;
            this.val = start;
        ...
        update() {
            this.frac = Math.min(1, this.get() / this.endtime);
            this.val = this.start + this.delta * (1 - Math.pow(1 - this.frac, this.exp));
            if (this.frac == 1) this.completed = true;
            return this.val;
        }
    }
    ...
    class TempMapImage extends Time {
        ...
        update() {
            if (this.get() >= this.expiry || this.event._sequenced == false || this.event._erased) {
                this.remove();
                return true;
            } else {
    ...
    class NudgePosition extends Time {
        constructor(obj, dir, amt, time, exp, ignore = true) {
            super(ignore);
            this.ignore = ignore;
            this.obj = obj;
            this.dir = dir; // 2 = down, 4 = left, 6 = right, 8 = up
            if (this.dir == 2) {
                this.original = this.obj._y;
                this.max = this.obj.y + amt;
            } else if (this.dir == 8) {
                this.original = this.obj._y;
                this.max = this.obj.y - amt;
            } else if (this.dir == 4) {
                this.original = this.obj._x;
                this.max = this.obj.x - amt;
            } else {
                this.original = this.obj._x;
                this.max = this.obj.x + amt;
            }
        ...
        update() {
            ...
            if (this.ease == null) {
                if (!this.halfpoint) {
                    // TODO first half
                    this.ease = new BMM.TIME.easeOut(this.current, this.max, this.exp, this.mid, this.ignore);
                } else {
                    // TODO second half
                    this.current = this.max;
                    this.ease = new BMM.TIME.easeOut(this.current, this.original, this.exp, this.mid, this.ignore);
                }
            }
            this.current = this.ease.update();
            if (this.dir == 2 || this.dir == 8) this.obj.setPosition(this.obj._x, this.current);
            else this.obj.setPosition(this.current, this.obj._y);
            if (this.ease.completed) {
                this.ease = null;
                if (!this.halfpoint) this.halfpoint = true;
                else this.endpoint = true;
            }
            if (this.endpoint) {
                if (this.dir == 2 || this.dir == 8) this.obj.setPosition(this.obj._x, this.original);
                else this.obj.setPosition(this.original, this.obj._y);
                this.completed = true;
            }
        }
    }

    BMM.TIME.pushNudge = function(obj, dir, amt, time, ease, ignore = true) {
        nudgedObjects.push(new NudgePosition(obj, dir, amt, time, ease, ignore));
    }
    ...
    BMM.TIME.pushMapImage = function(ev, path, xscale, yscale, opacity, expiry) {
        for (var i of mapspaceImages) {
            if (i.event == ev) {
                i.path = path;
                i.scale = [xscale, yscale];
                i.opacity = opacity;
                i.expiry = i.get() + expiry;
                $gameScreen.erasePicture(i.id);
                i.update();
                return;
            }
        }
    ...
    BMM.TIME.expiry = function() {
        ...
        for (var i = 0; i < spriteSwaps.length; i++) {
            if (spriteSwaps[i].update()) rmqueue.push(i);
        }
        rmqueue.reverse();
        for (var pos of rmqueue) {
            spriteSwaps.splice(pos, 1);
        }
        // Map Space Images
        rmqueue = [];
        ...
        for (var i = 0; i < mapspaceImages.length; i++) {
            if (mapspaceImages[i].update()) rmqueue.push(i);
        }
        rmqueue.reverse();
        for (var pos of rmqueue) {
            mapspaceImages.splice(pos, 1);
        }
        // Enemies - Nudge
        rmqueue = [];
        for (var i = 0; i < nudgedObjects.length; i++) {
            nudgedObjects[i].update()
            if (nudgedObjects[i].completed) rmqueue.push(i);
        }
        rmqueue.reverse();
        for (var pos of rmqueue) {
            nudgedObjects.splice(pos, 1);
        }
        // Player - Dash
        if (BMM.TIME.dashInterp != null) {
            var tempval = BMM.TIME.dashInterp.update();
            if (BMM.TIME.dashDir == 1) $gamePlayer.setPosition(tempval, $gamePlayer.y);
            else $gamePlayer.setPosition($gamePlayer.x, tempval);
            BMM.GLOBAL.interpreter.pluginCommand("CAM", [$gamePlayer.x.toString(), $gamePlayer.y.toString()]);
            if (BMM.TIME.dashInterp.completed) {
                BMM.TIME.dashInterp = null;
                BMM.TIME.dashDir = 0;
            }
        }
        // Radiant - Dash
        if (BMM.PT.isBoss() && BMM.PT.bossClass() == "radiant" && BMM.PT.radiantDash != null) {
            var tempval = BMM.PT.radiantDash.update();
            var rad = BMM.PT.getBoss();
            if (BMM.PT.radiantDashDir == 1) rad.event.setPosition(tempval, rad.y);
            else rad.event.setPosition(rad.x, tempval);
            if (BMM.PT.radiantDash.completed) {
                BMM.PT.radiantDash = null;
                BMM.PT.radiantDashDir = 0;
            }
            //console.log(rad.x + ", " + rad.y);
        }
        // Spectre - Radial Blur
        if (BMM.PT.isBoss() && BMM.PT.bossClass() == "spectre") {
            if (BMM.PT.spectreBlurStrength != null) {
                console.log(BMM.PT.spectreBlurStrength.update());
                BMM.GLOBAL.interpreter.pluginCommand("setFilter", ["specblur", "x", "x", "x", BMM.PT.spectreBlurStrength.update().toString()]);
                if (BMM.PT.spectreBlurStrength.completed) {
                    BMM.PT.spectreBlurStrength = null;
                    BMM.GLOBAL.interpreter.pluginCommand("setFilter", ["specblur", "x", "x", "x", "0"]);
                }
            }
        }
    }
    ...
    BMM.TIME.turnTimer = function() {
        ...
        if (BMM.TIME.time() >= BMM.TIME.nextTurn && !BMM.HYB.advanceReady) {
            //console.log("TIME OUT");
            if (BMM.GLOBAL.showTimer) AudioManager.playSe({name: 'TimeAdvance', pan: 0, pitch: 100, volume: 100});
            BMM.HYB.advanceTurn();
        }
    }
    ...
    BMM.TIME.reset = function() {
        ...
        if (BMM.PT.isBoss() && BMM.PT.bossClass() == "deceiver" && BMM.PT.bossStage() > 1 && BMM.PT.bossStage() < 4 && BMM.PT.bossHP() > 0) {
