...
(function() {
    ...
    BMM.TIME.activateDash = function(direction) {
        if (potentialDashMove[direction][0] != -1){//if it equals -1, cannot dash in direction
            ...
            if (direction == "up" || direction == "down") {
                ...
                BMM.TIME.dashInterp = new BMM.TIME.easeOut($gamePlayer.y, potentialDashMove[direction][1], 5, 250);
            } else if (direction == "left" || direction == "right") {
                BMM.TIME.dashDir = 1;
                BMM.TIME.playerEffectivePosition = [potentialDashMove[direction][0], $gamePlayer.y];
                BMM.TIME.dashInterp = new BMM.TIME.easeOut($gamePlayer.x, potentialDashMove[direction][0], 5, 250);
            }
    ...
    BMM.TIME.easeOut = class extends Time {
        constructor(start, end, exp, time) {
            super();
            this.start = start;
            this.val = start;
    ...
    BMM.TIME.turnTimer = function() {
        ...
        if (BMM.TIME.time() >= BMM.TIME.nextTurn) {
            ...
            AudioManager.playSe({name: 'TimeAdvance', pan: 0, pitch: 100, volume: 100});
            BMM.HYB.advanceTurn();
        }
    }
